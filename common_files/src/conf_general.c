...
void conf_general_setup_mc(mc_configuration *mcconf) {
	...
	/* VESC detection produces foc_current_kp/ki in SI units - kp = L*bw [V/A],
	 * ki = R*bw [V/(A*s)], see conf_general_calc_apply_foc_cc_kp_ki_gain() - but
	 * the ST current PI runs entirely in ADC counts: s16 current in, s16 voltage
	 * out. The two domains relate as
	 *
	 *   i_counts = i_A * CURRENT_FACTOR_A
	 *   v_counts = v_V * 65536 / Vbus            (32767 <-> Vbus/2, same s16
	 *                                             voltage scale the feed-forward
	 *                                             constants above are built on)
	 *
	 *   kp_counts = kp_V_per_A * 65536 / (Vbus * CURRENT_FACTOR_A)
	 *
	 * That factor was missing: the SI number was loaded straight into the loop,
	 * so with this board's CURRENT_FACTOR_A the current PI ran about 7.6x
	 * under-gained - a ~6.5 ms current-loop time constant where the detection
	 * routine designed 1.0 ms. Kp and Ki were under by the same factor, so the
	 * loop stayed correctly damped; it was slow, not marginal.
	 *
	 * Vbus is not known at config time and swings from 63 V charged to about
	 * 45 V empty, so the scale is fixed at NOMINAL_BUS_VOLTAGE_V. Residual error
	 * is +-17% of gain across that range: bandwidth ~857 rad/s at 63 V and
	 * ~1200 rad/s at 45 V, both far below the 16 kHz switching frequency. */
	#define FOC_CC_GAIN_SCALE (65536.0f / ((float)NOMINAL_BUS_VOLTAGE_V * (float)CURRENT_FACTOR_A))

	/* hKpGain/hKiGain are int16_t. The detected values land near 1280 and 630
	 * here, but clamp rather than let a large configured gain wrap to a negative
	 * one - a negative gain is positive feedback in the current loop. */
	float cc_kp = mcconf->foc_current_kp * FOC_CC_GAIN_SCALE * (float)TF_KPDIV;
	float cc_ki = mcconf->foc_current_ki * FOC_CC_GAIN_SCALE * (float)TF_KIDIV / (float)mcconf->foc_f_sw;
	utils_truncate_number(&cc_kp, 0.0f, (float)INT16_MAX);
	utils_truncate_number(&cc_ki, 0.0f, (float)INT16_MAX);

	PIDIqHandle_M1.hKpGain          	  = cc_kp;
	PIDIqHandle_M1.hKiGain                = cc_ki;
	...
	/* ---- Feed-forward constants, derived from mc_conf at every config load ----
	 *
	 * FF_VqdffComputation() (feed_forward_ctrl.o, MCSDK 5.4.8) computes, with
	 * w = hSpeed_dpp and Vb = AvBusVoltage_d / 2:
	 *
	 *   Vqdff.q += 16*(C2*w)/Vb + 2*(((w*Iqdref.d)/32768)*C1D)/Vb
	 *   Vqdff.d -=                2*(((w*Iqdref.q)/32768)*C1Q)/Vb
	 *
	 * which must equal the PMSM decoupling  Vq += we*Ld*Id + we*lambda,
	 * Vd -= we*Lq*Iq. Unit chain:
	 *
	 *   w   = we * 65536 / (2*pi*f_foc)          f_foc = TF_REGULATION_RATE [Hz]
	 *   Id  = I[A] * CURRENT_FACTOR_A            s16 current digits per amp
	 *   Vb  = Vbus[V] * BATTERY_VOLTAGE_GAIN / 2 raw ADC digits, halved in the SDK
	 *   V   = 65536 * Vphase / Vbus              s16 voltage, +-32767 <-> +-Vbus/2
	 *
	 * we and Vbus cancel on both axes, leaving
	 *
	 *   C1D = C1Q = 65536 * L      * pi*f_foc*G / (4*CF)
	 *   C2        = 65536 * lambda * pi*f_foc*G / (16*65536)
	 *
	 * The 65536 voltage scale is not documented by ST; it was solved from five
	 * MC Workbench reference projects, which agree on it to within 0.6%, and the
	 * K-independent ratio C2/C1D = lambda*CF/(262144*L) reproduces four of them
	 * to five significant figures. See docs/CHANGES.md.
	 *
	 * Ld = Lq: foc_motor_ld_lq_diff is not populated here and this motor has no
	 * saliency, which is also why the Workbench emits C1D == C1Q. */
	#define FF_C1_COEFF (FF_VOLTAGE_SCALE * 16384.0f * 3.14159265f * (float)TF_REGULATION_RATE * \
	                     (float)BATTERY_VOLTAGE_GAIN / (float)CURRENT_FACTOR_A)
	#define FF_C2_COEFF (FF_VOLTAGE_SCALE * 3.14159265f * (float)TF_REGULATION_RATE * \
	                     (float)BATTERY_VOLTAGE_GAIN / 16.0f)

	/* FF_Init() copies wDefConstant_* over the live wConstant_*, so it has to run
	 * first; FF_SetFFConstants() then installs the values computed here. It also
	 * stores pBus_Sensor, which FF_VqdffComputation() ignores in favour of the
	 * global pBusSensorM1 (set in MCboot) - passing it anyway keeps the handle
	 * honest for anything that reads it. */
	FF_Init(&FF_M1, &RealBusVoltageSensorParamsM1._Super, &PIDIdHandle_M1, &PIDIqHandle_M1);

	FF_TuningStruct_t ff_const;
	ff_const.wConst_1D = (int32_t)(FF_C1_COEFF * mcconf->foc_motor_l);
	ff_const.wConst_1Q = ff_const.wConst_1D;
	ff_const.wConst_2  = (int32_t)(FF_C2_COEFF * mcconf->foc_motor_flux_linkage);
	FF_SetFFConstants(&FF_M1, ff_const);
#endif

	mc_conf = *mcconf;

	/* Must be last: lo_current_max_scale was forced to 1.0 above, and the copy
	 * into mc_conf has just landed, so this is the point where the live config
	 * is complete. Re-applies the selected drive mode's current cap, which
	 * otherwise stayed at 1.0 until the next mode-button press - meaning any
	 * config write handed back full throttle authority while the display still
	 * read SLOW or DRIVE. Covers every caller: boot (:50), COMM_SET_MCCONF
	 * (VescCommand.c:470) and the detection paths (:524, :549, :564, :578). */
...
int conf_general_detect_apply_all_foc(float max_power_loss,	bool store_mcconf_on_success, bool send_mcconf_on_success, PACKET_STATE_t * phandle) {
	...
	*mcconf_old = *mcconf;
	...
	/* Deliberately sluggish gains for the measurement routines, expressed in the
	 * loop's own counts domain rather than SI - 0.0005 V/A would be meaningless
	 * for an 80 mOhm motor. FOC_CC_GAIN_SCALE is divided out so that
	 * conf_general_setup_mc() multiplies it back and the detection loop sees the
	 * same gains it saw before that scale existed.
	 *
	 * NOTE: these two assignments are currently INERT. The
	 * conf_general_setup_mc(mcconf) call below is commented out, and neither
	 * tune.c nor anything else pushes mcconf into the PID handles, so detection
	 * actually runs on whatever gains setup_mc() last installed - the stored
	 * foc_current_kp/ki. The scaling is kept anyway so that the values are
	 * correct if that call is ever restored; without it, restoring the call
	 * would hand the detection loop 7.6x the intended gain.
	 *
	 * They never persist either way: mcconf_old is restored before returning. */
	mcconf->foc_current_kp = 0.0005f / FOC_CC_GAIN_SCALE;
	mcconf->foc_current_ki = 1.0f / FOC_CC_GAIN_SCALE;
	...
	*mcconf = *mcconf_old;
	...
	/* The copy above puts lo_current_max_scale back to 1.0 and loses the drive
	 * mode's cap. conf_general_setup_mc() forces that field to 1.0 (see the top
	 * of it), copies the struct into the mc_conf global, and only then restores
	 * the mode scale through app_adc_apply_mode_scale() - which writes the
	 * GLOBAL, not the mcconf_old it was handed. So after the successful-detection
	 * path calls setup_mc(mcconf_old), mcconf_old still carries 1.0, and this
	 * copy writes that straight over the corrected global.
	 *
	 * The failure path below re-runs setup_mc() and recovers on its own, but the
	 * success path had nothing, leaving full current authority in every drive
	 * mode until the next mode-button press while the display still read SLOW or
	 * DRIVE. Re-applied unconditionally here: on the failure path setup_mc() has
	 * already done it and a second call is a no-op, and being unconditional
	 * keeps it correct if these branches are ever rearranged. */
	if (result < 0) {
		conf_general_setup_mc(mcconf);
	}
	app_adc_apply_mode_scale();

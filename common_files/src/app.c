...
void app_set_configuration(app_configuration *conf) {
	...
	switch (appconf.app_to_use) {
		...
		case APP_ADC:
			...
			/* Missing break. Falling through into APP_ADC_UART runs that case's
			 * kill logic, whose "old_app == APP_ADC -> task_app_kill()" branch
			 * destroys the task_app_init() above had just kept alive, after which
			 * task_cli_init() sees the now-NULL shared aux_uart.task_handle and
			 * starts tskCLI on the display's half-duplex UART instead. Harmless
			 * at boot only because the scheduler is not yet running there, so the
			 * kill blocks are skipped. */
			break;
		case APP_ADC_UART:
			if( xTaskGetSchedulerState() == taskSCHEDULER_RUNNING){
				if(old_app == APP_ADC){

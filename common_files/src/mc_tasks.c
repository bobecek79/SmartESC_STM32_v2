...
/* Low-passed current-PI output with the feed-forward contribution excluded.
 * Read by the PWM-off gate in VescToSTM.c - see FOC_CurrController(). */
qd_t AvVolt_qd_pi;
#endif
...
#if FEED_FORWARD_ENABLED
  /* Computed at medium frequency, not in the current loop: it costs two integer
   * divisions and depends only on speed and the current reference, both of which
   * move far slower than 16 kHz. This is also where ST calls it.
   * FOCVars[M1].bDriveInput is EXTERNAL on this firmware, so the reference is set
   * by the VESC layer rather than FOC_CalcCurrRef() - read it from FOCVars
   * directly so the call works either way. The function zeroes Vqdff by itself
   * when the bus reads 0. */
...
#if FEED_FORWARD_ENABLED
  /* Documented as "should be called before each motor restart"; zeroes Vqdff so
   * a stale contribution from the previous run cannot be applied on the first
   * PWM cycle after restart. */
  FF_Clear(&FF_M1);
#endif
...
__weak void FOC_CalcCurrRef(uint8_t bMotor)
{
  ...
  if(FOCVars[bMotor].bDriveInput == INTERNAL)
  {
    ...
    /* DC-side current from the AC side. ST's Clarke/Park is amplitude
     * invariant, so P = 3/2*(Vq*Iq + Vd*Id) and, dropping the d term,
     *
     *   I_batt = 1.5 * Vq * Iq / Vbus
     *
     * In the loop's own units, with i_c = I*CURRENT_FACTOR_A and
     * v_c = V*65536/Vbus (32767 <-> Vbus/2), Vbus cancels and the result is
     * already in the same current counts as max_i_batt/min_i_batt:
     *
     *   batt_i = 1.5 * i_c * v_c / 65536 = i_c * v_c / 43690.7
     *
     * This divided by 32768 - a factor of 2 where 1.5 belongs, so the estimate
     * ran 4/3 high and the limiter tripped at 0.75x l_in_current_max: 35 A
     * configured limited at 26.25 A.
     *
     * Written as a divide rather than "* 3 / 2" on purpose: i_c*v_c reaches
     * 1.07e9 and multiplying that by 3 would overflow int32.
     *
     * The two back-solves below invert this same relation and carry the same
     * constant; all three have to move together or the recovery current stops
     * matching the estimate that triggered it. int16 max_i_batt at its limit
     * gives 32767 * 43691 = 1.43e9, inside int32.
     *
     * No divide-by-zero guard is needed on AvVolt_qd.q: batt_i is proportional
     * to it, so a zero denominator cannot satisfy the comparisons that reach
     * the divisions. */
    #define BATT_I_SCALE 43691   /* 65536 / 1.5 */
...
inline uint16_t FOC_CurrControllerM1(void)
{
...
#if FEED_FORWARD_ENABLED
  /* Fault (b) fix. FW_M1.AvVolt_qd tracks the voltage actually applied, which
   * with feed forward on holds at back-EMF for as long as the wheel turns. The
   * PWM-off gate in VescToSTM_Iq_lim_hook() must instead test the part of the
   * applied voltage the current PI is responsible for, or the bridge never
   * freewheels on throttle release. Keep that FF-free signal here, using the
   * same first-order filter and bandwidth FW_DataProcess() applies, so the two
   * are directly comparable. The casts keep the arithmetic signed:
   * M1_VQD_SW_FILTER_BW_FACTOR is unsigned and would otherwise convert a
   * negative Vq to a large positive. */
  AvVolt_qd_pi.q = (int16_t)((Vqd.q + ((int32_t)M1_VQD_SW_FILTER_BW_FACTOR - 1) * (int32_t)AvVolt_qd_pi.q)
                             >> M1_VQD_SW_FILTER_BW_FACTOR_LOG);
  AvVolt_qd_pi.d = (int16_t)((Vqd.d + ((int32_t)M1_VQD_SW_FILTER_BW_FACTOR - 1) * (int32_t)AvVolt_qd_pi.d)
                             >> M1_VQD_SW_FILTER_BW_FACTOR_LOG);

  /* Adds Vqdff to the PID output, before Circle_Limitation so that the SUM is
   * what gets held inside the voltage limit rather than the PID part alone.
   * FF_VqdConditioning() also latches Vqd into FF_M1.VqdPIout. */
  Vqd = FF_VqdConditioning(&FF_M1, Vqd);
#endif

  /* Fault (c) fix. The deadband exists so the bridge is not asked for a duty
   * too small to switch cleanly, which is a statement about the voltage
   * actually commanded - so it belongs after the feed-forward add, not before.
   * Applied to the PID output alone it deleted the PI's correction in exactly
   * the regime feed forward is meant for: FF carries the voltage, the PI output
   * is small by design, and zeroing it removed the loop's only error term.
   * With FEED_FORWARD_ENABLED 0 there is nothing between this and the PID
   * output, so behaviour is unchanged from before feed forward existed. */
  //min duty limit
  ...
  /* Fault (a) fix: FF_DataProcess() is deliberately NOT called.
   *
   * Disassembly of feed_forward_ctrl.o in libmc-gcc_M3.lib shows
   * FF_VqdffComputation() ending with
   *     Vqdff.q = 2*(C1D*(w*Iqdref.d/32768))/Vb + 16*(C2*w)/Vb + VqdAvPIout.q
   * and FF_VqdConditioning() returning Vqd + Vqdff while latching VqdPIout=Vqd.
   * FF_DataProcess() is the only writer of VqdAvPIout, low-passing VqdPIout
   * into it. Calling all three therefore applies
   *     Vqd_PI + ff + LPF(Vqd_PI)
   * which in steady state is 2*Vqd_PI + ff - the current loop's proportional
   * gain doubled, against PI gains that conf_general_calc_apply_foc_cc_kp_ki_gain()
   * derives assuming the applied voltage is the PI output.
   *
   * FF_M1 is a designated initialiser in mc_config.c, so VqdAvPIout starts at
   * zero, and with FF_DataProcess() never called nothing else writes it. The
   * term drops out and Vqdff carries the feed-forward contribution only. */
  return(hCodeError);
}

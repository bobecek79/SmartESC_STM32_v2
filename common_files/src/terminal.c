...
void terminal_top(PACKET_STATE_t * phandle){
    ...
    if(taskStats){
       ...
       uint32_t foc_load = pMCI[M1]->pFOCVars->cycles_max * PWM_FREQUENCY * 100 / CPU_CLOCK;
        ...
        /* Low-water mark since boot. This is the number to size configTOTAL_HEAP_SIZE
         * against, not the "Used" figure above: the largest consumer is the VESC Tool
         * scope burst (VescCommand.c:557-564), which is transient and will almost never
         * be in flight at the moment you type "top".
         * Slightly pessimistic by design - the taskStats pvPortMalloc() at the top of
         * this function has already run, so its allocation is included in the peak. */
        uint32_t heapMinEverFree = xPortGetMinimumEverFreeHeapSize();
        commands_printf(phandle, "Mem peak: MinEverFree: %db PeakUsed: %db (%d%%)", heapMinEverFree, configTOTAL_HEAP_SIZE - heapMinEverFree, ((configTOTAL_HEAP_SIZE - heapMinEverFree) * 100) / configTOTAL_HEAP_SIZE);

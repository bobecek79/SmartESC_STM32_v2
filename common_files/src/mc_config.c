...
CURR_Handle_t CurrentSensorParams =
{
  ...
  .CurrRegConv =
  {
    .regADC = ADC1,
    .channel = MC_ADC_CHANNEL_0,
    /* Must match TempSensorParamsM1.TempRegConv.samplingTime above: both handles
     * sit on channel 0 and share one SMPR field. RCM_RegisterRegConv() writes it
     * once per registration and RCM_ExecRegularConv() never reprograms it, so if
     * both handles ever register, the last one to register wins for both. With
     * CURR_SENSOR_TYPE == VIRTUAL_SENSOR (M365) CURR_Init() returns before
     * registering and this value is unused; it is kept in sync so that flipping
     * to REAL_SENSOR cannot silently revert channel 0 to 1.5 cycles. Was 0. */
    .samplingTime = M1_TEMP_SAMPLING_TIME,
  }
};
...
/**
  * @brief  Feed forward parameters Motor 1
  *
  * The three constants are left at 0 here and computed at config load in
  * conf_general_setup_mc(), because they depend on foc_motor_l and
  * foc_motor_flux_linkage, which are runtime fields of mc_conf populated by
  * VESC Tool detection - not compile-time values. FF_Init() copies
  * wDefConstant_* into the live wConstant_*, so it must run BEFORE
  * FF_SetFFConstants() or it would overwrite them with these zeros.
  */
FF_Handle_t FF_M1 =
{
  .hVqdLowPassFilterBW    = M1_VQD_SW_FILTER_BW_FACTOR,
  .wDefConstant_1D        = (int32_t)0,
  .wDefConstant_1Q        = (int32_t)0,
  .wDefConstant_2         = (int32_t)0,
  .hVqdLowPassFilterBWLOG = M1_VQD_SW_FILTER_BW_FACTOR_LOG
};
#endif

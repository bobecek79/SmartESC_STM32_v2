...
#include "drive_parameters.h"   /* FEED_FORWARD_ENABLED */
...
/* Low-passed current-PI output, feed-forward contribution excluded. The PWM-off
 * gate must not test FW_M1.AvVolt_qd when feed forward is on, because that holds
 * at back-EMF while the wheel turns. Defined in mc_tasks.c. */
extern qd_t AvVolt_qd_pi;
#endif

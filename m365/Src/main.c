...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  /* NOTE: this value never reaches a conversion. Every regular conversion in
   * this firmware goes through the RegularConversionManager, and
   * RCM_RegisterRegConv() (regular_conversion_manager.c) both disables this
   * 5-channel scan sequence (LL_ADC_REG_SEQ_SCAN_DISABLE) and reprograms the
   * channel's SMPR field from the RegConv_t handle. The values that actually
   * take effect are M1_VBUS_SAMPLING_TIME (ch2) and M1_TEMP_SAMPLING_TIME (ch0)
   * in drive_parameters.h. Kept in sync here so this does not mislead.
   * Ranks 3/4/5 below (ch6/7/9, phase V-sense) are never converted at all. */
